...
FREQUENCIES = {"daily": ("每日", 1), "weekly": ("每周", 7), "biweekly": ("双周", 14), "monthly": ("每月", 30)}
...
def next_remind(last_progress_iso: str, frequency: str) -> str:
    days = FREQUENCIES.get(frequency, FREQUENCIES["weekly"])[1]
...
async def items_list(ctx: Ctx = Depends(get_ctx), project: str = "", status: str = "open", q: str = "", mine: int = 0):
    ...
    now = db.utcnow()
    return ctx.render("items/list.html", items=[{"i": r, "state": _due_state(r, now)} for r in rows], project=project, status=status, q=q, mine=mine, FREQUENCIES=FREQUENCIES, STATUS=STATUS)
...
def _read_form(form) -> dict:
    ...
    return {"project": project, "title": title[:200], "description": str(form.get("description") or "").strip(), "frequency": frequency, "due_date": due or None}
...
async def item_new(request: Request, ctx: Ctx = Depends(get_ctx)):
    ...
    now = db.utcnow()
    item_id = db.insert(ctx.conn, "key_items", {**data, "status": "open", "created_by": ctx.user["id"], "created_at": now, "updated_by": ctx.user["id"], "updated_at": now, "last_progress_at": now, "next_remind_at": next_remind(now, data["frequency"])})
...
async def item_edit(item_id: int, request: Request, ctx: Ctx = Depends(get_ctx)):
    ...
    data = _read_form(form)
    changes = []
    if data["frequency"] != item["frequency"]:
        changes.append(f"提醒频率 {FREQUENCIES[item['frequency']][0]} → {FREQUENCIES[data['frequency']][0]}")
        data["next_remind_at"] = next_remind(item["last_progress_at"], data["frequency"])
...
async def item_detail(item_id: int, ctx: Ctx = Depends(get_ctx)):
    ...
    return ctx.render(
        ...
        reads=reads, reqs=reqs, FREQUENCIES=FREQUENCIES, STATUS=STATUS, UPDATE_KINDS=UPDATE_KINDS, state=_due_state(item, db.utcnow()),
...
async def item_progress(item_id: int, request: Request, ctx: Ctx = Depends(get_ctx)):
    ...
    now = db.utcnow()
    db.update(ctx.conn, "key_items", item_id, {"last_progress_at": now, "next_remind_at": next_remind(now, item["frequency"]), "updated_by": ctx.user["id"], "updated_at": now})
...
async def item_status(item_id: int, request: Request, ctx: Ctx = Depends(get_ctx)):
    ...
    if done:
        ...
        db.update(ctx.conn, "key_items", item_id, {"status": "open", "completed_at": None, "completed_by": None, "last_progress_at": now, "next_remind_at": next_remind(now, item["frequency"]), "updated_by": ctx.user["id"], "updated_at": now})
...
def run_reminders(cfg: Config, conn: sqlite3.Connection, notifier: Notifier, base_url: str, now: str | None = None) -> int:
    ...
    for item in due:
        ...
        body = f"负责人：{'、'.join(owners) or '—'}；提醒频率：{FREQUENCIES[item['frequency']][0]}" + (f"；截止 {item['due_date']}" if item["due_date"] else "") + "。负责人更新进展后计时会重置。"
        ...
        db.update(conn, "key_items", item["id"], {"next_remind_at": next_remind(now, item["frequency"])})
        n += 1
    return n
