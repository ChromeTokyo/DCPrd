...
- **重点事项** `/items`：项目 + 标题 + 描述 + 附件（多个）+ 负责人（多选）+ 汇报对象（多选，也可写进展）+ 提醒频率（日/周/双周/月，滚动周期）+ 可选截止日期 + 关联需求。进展记录时间线，每条进展显示各接收人的已读/未读；写进展重置计时；到期未更新由后台每分钟检查一次，提醒负责人与汇报对象，并把下次提醒推一个周期；标记完成后停止提醒。所有登录用户可新增/编辑/写进展/完成，删除仅创建人与管理员。附件在 `data/items/<id>/`。
